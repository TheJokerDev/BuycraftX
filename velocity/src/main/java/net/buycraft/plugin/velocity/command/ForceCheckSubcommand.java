...
public class ForceCheckSubcommand implements Subcommand {
    ...
    public void execute(CommandSource sender, String[] args) {
        ...
        if (plugin.getDuePlayerFetcher().inProgress()) {
            sender.sendMessage(Component.text(plugin.getI18n().get("already_checking_for_purchases")).color(TextColor.color(255, 0, 0)));
            return;
        }

        plugin.getPlatform().executeAsync(() -> plugin.getDuePlayerFetcher().run(false));
        sender.sendMessage(Component.text(plugin.getI18n().get("forcecheck_queued")).color(TextColor.color(0, 255, 0)));
    }
